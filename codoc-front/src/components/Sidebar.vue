...
import maisonIcon from '@/assets/icons/maison.svg'
import randomIcon from '@/assets/icons/random.svg'
import sortirIcon from '@/assets/icons/sortir.svg'
import editerIcon from '@/assets/icons/editer.svg'
import reglagesIcon from '@/assets/icons/reglages.svg'
import loginIcon from '@/assets/icons/ajout-dutilisateur.svg'
import utilisateurIcon from '@/assets/icons/utilisateur.svg'
...
<template>
  <aside class="flex h-screen w-64 flex-col justify-between border-r border-gray-200 bg-white p-6 dark:border-codoc-card dark:bg-codoc-dark">
    <div>
      ...
      <nav class="space-y-1">
        ...
        <template v-else>
          <RouterLink to="/login"
            class="flex items-center gap-3 rounded px-3 py-2 text-gray-700 transition-colors hover:bg-gray-100 dark:text-codoc-cyan dark:hover:bg-codoc-card dark:hover:text-white">
            <img :src="loginIcon" alt="" class="w-5" /> Login
          </RouterLink>
          <RouterLink to="/register"
            class="flex items-center gap-3 rounded px-3 py-2 text-gray-700 transition-colors hover:bg-gray-100 dark:text-codoc-cyan dark:hover:bg-codoc-card dark:hover:text-white">
            <img :src="utilisateurIcon" alt="" class="w-5" /> Register
          </RouterLink>
        </template>
      </nav>
    </div>
    ...
    <div class="space-y-3">
      <DarkModeToggle />
      <template v-if="auth.isAuthenticated">
        <p class="text-sm text-gray-500 dark:text-codoc-gray">{{ auth.user?.username }}</p>
        <button @click="handleLogout"
          class="flex w-full items-center gap-3 rounded px-3 py-2 text-left text-sm text-gray-700 transition-colors hover:bg-gray-100 dark:text-codoc-cyan dark:hover:bg-codoc-card">
          <img :src="sortirIcon" alt="" class="w-5" /> Logout
        </button>
      </template>
    </div>
  </aside>
